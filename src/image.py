...
import numpy as np
import random
from car_like import move_car_like
from src.car_like import get_action_time
...
class Image:
    ...
    def __add_car_like_curve(self, start, goal, steering_angle, car_length, n_points=10, color=(87, 126, 137),
                             width=0.005):
        time = get_action_time(start, goal, steering_angle, car_length)

        path = []
        for i in range(n_points + 1):
            path.append(move_car_like(start[:3], steering_angle, car_length, i * (time / n_points)))
    ...
    def add_path_car_like(self, path, car_length=0.1, resolution=20, color=(253, 198, 117), width=0.005):
        """
        Adds RRT tree generated by the car like algorithm to the image.

        Args:
            path: path returned by the solve_car_like function
            car_length: length of the car-like model
            resolution: number of points approximating one car-like curve
            color: color of the polygon - tuple (R, G, B) with the RGB color intensities (each from 0 to 255)
            width: width of the path
        """

        self.add_circle(path[0, :2], 1.8 * width, color)
        for state in path[1:-1]:
            self.add_circle(state[:2], 1.2 * width, color)
        self.add_star(path[-1, :2], 3 * width, color)

        for i in range(len(path) - 1):
            self.__add_car_like_curve(path[i], path[i + 1], path[i, 3], car_length, resolution, color, width)

    def add_paths_car_like(self, paths, car_length=0.1, resolution=20, use_one_color=False, color=(87, 126, 137), width=0.005):
        """
        Adds multiple paths to the image.

        Args:
            paths: numpy tensor with paths - shape: (number of paths, number of points in path, 4)
            car_length: length of the car-like model
            resolution: number of points approximating one car-like curve
            use_one_color: set to True if all paths should have the same color - specified via the color param
            color: color of the paths (if use_one_color is set to True)
        ...
        for i, path in enumerate(paths):
            ...
            if not use_one_color:
                if i < len(color_palette):
                    ...
                    # generating random color instead
                    c = (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
            self.add_path_car_like(path, car_length, resolution, c, width)

    def add_tree_car_like(self, tree, car_length=0.1, resolution=20, color=(87, 126, 137),
                          width=0.0045):
        """
        Adds RRT tree generated by the car like algorithm to the image.

        Args:
            tree: Tree object returned by the solve_car_like function
            car_length: length of the car-like model
            resolution: number of points approximating one car-like curve
            color: color of the polygon - tuple (R, G, B) with the RGB color intensities (each from 0 to 255)
            width: width of the tree edges
        """

        def __plot_recursive(root):
            self.add_circle(root.state[:2], 1.2 * width, color)
            for child in root.children:
                self.__add_car_like_curve(root.state, child.state, child.state[3], car_length, resolution, color, width)
                __plot_recursive(child)

        __plot_recursive(tree.get_root())
