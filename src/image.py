...
class Image:
    ...
    def __add_car_like_curve(self, start, steer_angle, time, car_length, n_points=10, color=(87, 126, 137),
                             width=0.005):
        path = []
        for i in range(n_points + 1):
            path.append(move_car_like(start[:3], steer_angle, car_length, i * (time / n_points)))
    ...
    def add_path_car_like(self, path, car_length=0.1, time_per_action=0.1, resolution=20, color=(253, 198, 117), width=0.005):
        """
        ...
        Args:
            ...
            time_per_action: action_time used when calling solve_car_like
        ...
        for sample in path[:-1]:
            self.__add_car_like_curve(sample, sample[3], time_per_action, car_length, resolution, color, width)

    def add_tree_car_like(self, tree, car_length=0.1, time_per_action=0.1, resolution=20, color=(87, 126, 137),
                          width=0.005):
        """
        Adds RRT tree generated by the car like algorithm to the image.
        ...
        def __plot_recursive(root):
            ...
            for child in root.children:
                self.__add_car_like_curve(root.state, child.state[3], time_per_action, car_length, resolution, color,
                                          width)
